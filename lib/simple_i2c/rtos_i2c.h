...
inline void RTOS_I2C::init_i2c1() {
  ...
  HAL_I2C_RegisterCallback(&hi2c_, HAL_I2C_MSPINIT_CB_ID, RTOS_I2C::i2c1_msp_init);
  HAL_I2C_Init(&hi2c_);
...
inline void RTOS_I2C::i2c1_msp_init(I2C_HandleTypeDef* hi2c) {
...
inline bool RTOS_I2C::write(uint8_t address, uint8_t* data, size_t len) {
  utils::Lock lck(mtx_);
  return HAL_I2C_Master_Transmit(&hi2c_, address, data, len, timeout_) == HAL_OK;
}

inline bool RTOS_I2C::read(uint8_t address, uint8_t* data, size_t len) {
  utils::Lock lck(mtx_);
  return HAL_I2C_Master_Receive(&hi2c_, address, data, len, timeout_) == HAL_OK;
}

inline bool RTOS_I2C::write_register(uint8_t address, uint8_t reg_addr, uint8_t* data, size_t len) {
  utils::Lock lck(mtx_);
  return HAL_I2C_Mem_Write(&hi2c_, address, reg_addr, 1, data, len, timeout_) == HAL_OK;
}

inline bool RTOS_I2C::read_register(uint8_t address, uint8_t reg_addr, uint8_t* data, size_t len) {
  utils::Lock lck(mtx_);
